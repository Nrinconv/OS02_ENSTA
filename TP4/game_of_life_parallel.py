...
if __name__ == '__main__':
    ...
    # rank 0 handles visualization
    if rank == 0:
        pg.init()
        print(f"Pattern selected: {choice}")
        print(f"Screen resolution: {resx}x{resy}")
        ...
        while loop:
            comm.send("request_data", dest=1)  # request updated grid from computation process
            grid.cells = comm.recv(source=1)  # receive updated grid
            t1 = time.time()
            appli.draw()  # draw grid on screen
            t2 = time.time()

            # record iteration metrics
            iteration_time = t2 - t1
            time_per_iter.append(iteration_time)
            total_time += iteration_time
            live_cells.append(np.sum(grid.cells))
            n_iterations += 1
            ...
            print(f"Iterations: {n_iterations}, Rendering time: {t2 - t1:.6f}s", end='\r')

        comm.send("exit", dest=1)  # signal computation process to stop
        end_time = time.time()

        # compute simulation statistics
        total_sim_time = end_time - start_time
        avg_time_per_iter = total_time / n_iterations if n_iterations > 0 else 0
        std_time_per_iter = np.std(time_per_iter) if n_iterations > 1 else 0
        avg_live_cells = np.mean(live_cells)

        # print execution summary
        print("\n--- Simulation Summary ---")
        print(f"Grid size: {grid.dimensions[0]} rows x {grid.dimensions[1]} columns")
        print(f"Initial number of living cells: {len(init_pattern[1])}")
        print(f"Total iterations: {n_iterations}")
        print(f"Total simulation time: {total_sim_time:.2f}s")
        print(f"Average time per iteration: {avg_time_per_iter:.6f}s")
        print(f"Iteration time standard deviation: {std_time_per_iter:.6f}s")
        print(f"Average living cells per iteration: {avg_live_cells:.2f}")

        pg.quit()

    # rank 1 handles computation
    elif rank == 1:
        grid = Grille(*init_pattern)

        while True:
            msg = comm.recv(source=0)  # wait for request from visualization process

            if msg == "exit":
                break  # terminate loop if exit signal is received

            grid.compute_next_iteration()  # compute next generation

            comm.send(grid.cells, dest=0)  # send updated grid to visualization process