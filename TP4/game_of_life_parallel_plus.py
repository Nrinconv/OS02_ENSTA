...
if __name__ == '__main__':
    ...
    # split processes into separate communicators
    ...
    if rank == 0:
        ...
        while loop:
            # request updated grid from computing processes
            ...
            live_cells.append(np.sum(appli.grid.cells))
            n_iterations += 1
            ...
            print(f"Iterations: {n_iterations}, Rendering time: {t3 - t2:2.2e} seconds", flush=True, end='\r')
    ...
    # processes rank >= 1 handle computation
    elif rank >= 1:
        # initialize local grid for each computing process
        ...
        while loop:
            ...
            # compute next iteration and update ghost cells
            grid.compute_next_iteration()
            grid.update_ghost_cells()
            t2 = time.time()

            # gather computed grid parts
